...
ggsave(
  raw_ablation_plot,
  file = file.path(out_dir, "raw_ablation_plot.png"),
  width = 12,
  height = 10,
  dpi = 600
)
...
ggsave(
  main_elements,
  file = file.path(out_dir, "main_elements.png"),
  width = 10,
  height = 10,
  bg = "white",
  dpi = 600
...
ggsave(
  other_elements,
  file = file.path(out_dir, "other_elements.png"),
  width = 10,
  height = 10,
  dpi = 600
)
...
```{r}
#| label: tbl-rf
#| tbl-cap: "RF model confusion matrix of the 602 signatures tested. Actual shown in columns and predicted shown in rows. The diagonal bold numbers are the correct predictions for each site."
#| echo: false
#| warning: false
#| message: false
...
conf_display <- conf_matrix[, !colnames(conf_matrix) %in% "class.error"]

# Add class error as % column
class_error_pct <- paste0(round(conf_matrix[, "class.error"] * 100, 1), "%")

conf_display <- cbind(
  as.data.frame(conf_display),
  `Class error` = class_error_pct
)

# Find diagonal positions for bolding
n_sites <- nrow(conf_display)
...
# Bold diagonal cells using explicit loop
for (i in seq_len(n_sites)) {
  bold_vec <- rep(FALSE, n_sites)
  bold_vec[i] <- TRUE
  tbl_rf <- tbl_rf |>
    column_spec(
      column = i + 1,
...
conf_display <- cbind(conf_wide, `Class error` = class_error_pct)

n_sites <- ncol(conf_wide) # number of site columns only, not rows

# Build base table
tbl_lda <- conf_display |>
  kbl(
    booktabs = TRUE,
    align = "r",
    escape = FALSE
  ...
  add_header_above(
    c(" " = 1, "Actual site" = n_sites, " " = 1)
  ) |>
  row_spec(0, bold = TRUE) |>
  footnote(
    general = paste0(
      "Overall accuracy: ",
      round(mean_accuracy * 100, 1),
      "% (",
      round(sum_accuracy),
      " of ",
      num_folds,
      " correct) — SD: ",
      round(sd_accuracy * 100, 1),
      "%, SE: ",
      round(se_accuracy * 100, 1),
      "%"
    ),
    general_title = ""
  )
...
ggsave(
  lda_plot,
  file = file.path(out_dir, "lda_plot.png"),
  width = 10,
  height = 10,
  bg = "white",
  dpi = 600
...
class_table
mean_accuracy_lda <- mean(accuracy_scores) * 100
mean_accuracy_lda
```
...
ggsave(
  NNCA_tree_plot,
  file = file.path(out_dir, "NNCA_tree_plot.png"),
  width = 4,
  height = 5,
  dpi = 600
)
...
n_cl <- length(cluster_levels) # define here
...
combined_display |>
  kbl(
    ...
    align = "r",
    escape = FALSE,
    col.names = c("Predicted", cluster_levels, cluster_levels),
    row.names = FALSE
  ) |>
  kable_styling(
    full_width = FALSE,
  ...
  add_header_above(
    c(" " = 1, "RF" = n_cl, "LDA" = n_cl)
  ) |>
  row_spec(0, bold = TRUE) |>
  footnote(
    general = paste0(
      "RF overall accuracy: ",
      round(accuracy_rf_nnca * 100, 1),
      "% — ",
      "LDA overall accuracy: ",
      round(accuracy_lda_nnca * 100, 1),
      "%"
    ),
    general_title = ""
  )
...
# Plot LDA results
ggplot(Juv_NNCA2, aes(x = LD1, y = LD2)) +
  geom_point(aes(fill = NNCA), color = 'black', shape = 21) +
...
ggsave(
  LDA_NNCA,
  file = file.path(out_dir, "LDA_NNCA.png"),
  width = 5,
  height = 4,
  bg = "white",
  dpi = 600
...
tbl |>
  kbl(booktabs = TRUE, align = c("l", "l", "r", "r", "c", "l")) |>
  kable_styling(
    full_width = FALSE,
